...
const ClerkSignUpWidget = dynamic(
  () =>
    import("@/components/auth/clerk-auth-widgets").then(
      (mod) => mod.ClerkSignUpWidget,
    ),
  { ssr: false },
);
      ...
      <Card className="max-w-md">
        <CardContent className="space-y-4 p-8 text-center">
          ...
          <p className="text-sm text-muted-foreground">
            Sign up is not configured in this environment. The application runs
            with a clearly labeled demo auth context, so you can explore the
            workspace without an account.
          </p>
