...
const BASE_TIME = Date.parse("2026-05-30T13:00:00Z");
...
export function generateCustomer(input: GenerateCustomerInput): GeneratedCustomer {
  ...
  const signals: Signal[] = archetypes.map((archetype, i) => ({
    ...
    receivedAt: new Date(BASE_TIME - i * 3600_000 - index * 60_000).toISOString(),
  }));
  ...
  const opportunity: Opportunity = {
    ...
    updatedAt: new Date(BASE_TIME - index * 60_000).toISOString(),
  };
  ...
  const customer: Customer = {
    ...
    lastActionAt: new Date(BASE_TIME - index * 60_000).toISOString(),
    riskFlags:
